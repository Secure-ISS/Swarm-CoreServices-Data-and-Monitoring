...
class AlertManager:
    ...
    def send_alert(self, title: str, message: str, level: AlertLevel, details: Optional[Dict] = None):
...
class HealthChecker:
    ...
    def __init__(self, metrics: HealthCheckMetrics, alert_manager: AlertManager):
        ...
        self.thresholds = {
            ...
            'response_time_critical': float(os.getenv('HEALTH_CHECK_RESPONSE_TIME_CRITICAL', '5.0')),
        ...
        try:
            ...
            if response_time >= self.thresholds['response_time_critical']:
                self.alerts.send_alert(
                    ...
                    AlertLevel.CRITICAL,
                    {'response_time': f"{response_time:.2f}s", 'threshold': f"{self.thresholds['response_time_critical']}s"}
                )
                ...
                self.alerts.send_alert(
                    ...
                    f"Health check took {response_time:.2f}s",
                    AlertLevel.WARNING,
                    {'response_time': f"{response_time:.2f}s", 'threshold': f"{self.thresholds['response_time_warning']}s"}
                )
...
def main():
    ...
    state_file = Path(os.getenv(
        ...
        '/tmp/postgres-health-check.state.json'
    ))
    ...
    if output_format == 'json':
        ...
        print(f"# HELP postgres_health_status Overall health status (0=healthy, 1=warning, 2=critical)")
        ...
        print(f"postgres_health_response_time_seconds {results['metrics']['total_response_time_seconds']}")
